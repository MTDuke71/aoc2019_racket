...
.venv\Scripts\python.exe -m pip install networkx matplotlib pydot   # guide figures only (pythoniz_*.py)
...
figure scripts (`pythoniz_dayNN.py`) also need Graphviz's `dot` on PATH
