...
> Code: [python/day21.py](../../python/day21.py) and
> [python/day21_synth.py](../../python/day21_synth.py). Tests:
> [python/tests/test_day21.py](../../python/tests/test_day21.py) (28 tests).
> Statement: [day21.md](day21.md).
>
> **Answers: Part 1 = 19354392, Part 2 = 1139528802** (produced by the
> machine; `LOCKED` pending submission).
...
* The hull itself never appears — the only glimpses the machine gives are
  the 17-tile windows in failure renderings. Running the statement's `NOT D J`
  suicide against the real machine reproduces the statement's example
  rendering *exactly*, hull line `#####.###########` included
  (`test_failed_survey_renders_the_last_moments`) — the example was
  evidently generated by this very console.
...
* **The machine**: a failing survey yields no damage and renders the last
  moments; CRLF fixtures; `check_locked` (currently reporting both parts as
  UNVERIFIED skips, per the platform rule).
...
idea — mapping the hidden hull by deliberately crashing droids and
stitching the 17-tile death windows — would make a fine companion script if
this day ever gets a revisit; the single-step Intcode debugger remains the
post-year capstone candidate.
