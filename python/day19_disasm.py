...
SECTIONS = {
    0: "0000 .. 0220 -- main: read (x, y), evaluate the predicate, answer",
    221: "0221 .. 0224 -- variables (four zero-initialised cells)",
    225: "0225 .. 0258 -- apply(f, a, b, c): indirect call via a self-patched jump",
    259: "0259 .. 0281 -- abs(v): branch-free absolute value",
    282: "0282 .. 0302 -- reject_neg(v): a negative coordinate answers 0 and halts",
...
def full_listing(mem: list[int]) -> str:
    ...
    while addr < len(mem):
        if addr in SECTIONS:
            if lines:
                lines += ["```", ""]
            lines += [f"## {SECTIONS[addr]}", "", "```"]
        if addr in SYMBOLS:
            lines.append(f"{addr:04d}  {mem[addr]:<22} .var {SYMBOLS[addr]} = {mem[addr]}")
            covered.add(addr)
            addr += 1
            continue
        text, op, length = decode(mem, addr)
        ...
        if not note and op in (5, 6):
            mode2 = (mem[addr] // 1000) % 10
            target = mem[addr + 2]
            if mem[addr : addr + 3] in ([2105, 1, 0], [2106, 0, 0]):
                note = "return"
            elif mode2 == 1 and target in SUBROUTINES:
                note = f"call {SUBROUTINES[target].split()[0]}"
            elif mode2 == 0:
                note = f"indirect call through [{target}] = {mem[target]}"
        cells = " ".join(str(v) for v in mem[addr : addr + length])
        body = f"{addr:04d}  {cells:<22} {text}"
        lines.append(f"{body:<52}; {note}" if note else body)
    ...
    intro = (
        ...
        f"> `python python/day19_disasm.py --full`. Not committed: the raw cells\n"
        f"> republish the puzzle input (see `.gitignore`).\n"
        f">\n"
        f"> The analysis behind the annotations is in\n"
        f"> [day19_disassembly.md](day19_disassembly.md); this file is the evidence\n"
        f"> for it, in address order. Notation: `[a]` position, `#n` immediate,\n"
        f"> `rb[+n]` relative; an `=name` suffix marks a known variable.\n\n"
        f"At run time the stack occupies 424 upward; nothing else is in the file.\n\n"
    )
    return intro + "\n".join(lines) + "\n"
